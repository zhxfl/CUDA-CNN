...
*/
__global__ void g_feedforward_1(
	double** arrayS,
	double** arrayW,
	double** arrayB,
	double* conv,
...
*	threads: dim3(min(convOutputSize * convOutputSize, 512));
*/
__global__ void g_feedforward_2(
	double* pool1,
	double** arrayW,
	double** arrayB,
	double* conv2,
...
void ConvNCFM::feedforward()
{
	...
	if(inputs_1){
		...
		g_feedforward_1<<<block, thread>>>(inputs_1->m_devPoint,
			w.m_devPoint, 
			b.m_devPoint,
			outputs->devData,
			inputDim,
		...
		getLastCudaError("convNCFM:g_feedforward_1");
	}
	else if(inputs_2){
		dim3 block = dim3(batch, outputAmount, Config::instance()->getChannels());
		dim3 thread= dim3(min(outputDim * outputDim, 512));
		g_feedforward_2<<<block, thread>>>(inputs_2->devData,
			w.m_devPoint,
			b.m_devPoint,
			outputs->devData,
			inputDim,
			...
			outputs->getArea());
		checkCudaErrors(cudaDeviceSynchronize());
		getLastCudaError("convNCFM::g_feedforward_2");
	}
	else{
		printf("ConvNCFM init error\n");
		exit(0);
...
void ConvNCFM::backpropagation()
{
	...
	if(inputs_2){
		...
		g_backpropagation<<<block, thread>>>(
			curDelta->devData,
			w.m_devPoint,
			preDelta->devData,
			outputDim,
			...
			preDelta->getArea());
		checkCudaErrors(cudaDeviceSynchronize());
		getLastCudaError("ConvNCFM::g_backpropagation");
	}
}
